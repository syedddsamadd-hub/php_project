...
// Filter
if ($category !== 'all') $articles = array_filter($articles, fn($a) => $a['cat'] === $category);
if ($search) $articles = array_filter($articles, fn($a) => stripos($a['title'],$search)!==false || stripos($a['excerpt'],$search)!==false);
  ...
  <section style="background:linear-gradient(135deg,#0097A7,#00695C);padding:60px 0;color:white;">
    <div class="container">
      <div class="row align-items-center">
        <div class="col-lg-7">
          <span class="badge-accent" style="background:rgba(255,255,255,0.2);color:white;border:1px solid rgba(255,255,255,0.3);">Stay Informed</span>
        ...
        <div class="col-lg-5 mt-3 mt-lg-0">
          <form method="GET" action="medical-news.php" class="input-group">
            <input type="text" name="search" class="form-control" placeholder="Search news articles..." value="<?php echo htmlspecialchars($search); ?>" style="border-radius:var(--radius-sm) 0 0 var(--radius-sm);border:none;padding:14px 18px;" />
            <?php if ($category !== 'all'): ?><input type="hidden" name="cat" value="<?php echo htmlspecialchars($category); ?>" /><?php endif; ?>
            <button type="submit" class="btn" style="background:var(--primary);color:white;border-radius:0 var(--radius-sm) var(--radius-sm) 0;padding:0 20px;"><i class="fas fa-search"></i></button>
          </form>
        </div>
      </div>
    </div>
  ...
  <section class="section-padding">
    <div class="container">
        ...
        <div class="row g-0">
          <div class="col-lg-6">
            <div class="news-featured-body h-100 d-flex flex-column justify-content-center">
              ...
              <a href="#" class="btn-outline-care" style="width:fit-content;"><i class="fas fa-arrow-right"></i> Read Full Article</a>
            </div>
          </div>
          <div class="col-lg-6">
            <div style="height:100%;min-height:280px;background:rgba(255,255,255,0.1);display:flex;align-items:center;justify-content:center;font-size:6rem;opacity:0.3;">
      ...
      <div class="text-center py-5">
        ...
        <h5 class="mt-3" style="color:var(--text-muted);">No articles found<?php echo $search ? ' for "'.htmlspecialchars($search).'"' : ''; ?>.</h5>
        <a href="medical-news.php" class="btn-primary-care mt-3">View All News</a>
      </div>
      <?php else: ?>
      <div class="row g-4">
        <?php $i=0; foreach ($articles as $art): $i++; ?>
        <div class="col-md-6 col-lg-4 news-card-item animate-on-scroll" style="transition-delay:<?php echo (($i-1)%3)*0.07; ?>s">
          <div class="news-card">
            <div class="news-card-img" style="background:<?php echo $art['bg']; ?>">
              <i class="fas fa-<?php echo $art['icon']; ?>" style="color:<?php echo $art['ic']; ?>"></i>
              <span class="news-category"><?php echo ucfirst($art['cat']); ?></span>
            </div>
            <div class="news-card-body">
              <div class="news-meta">
                <span><i class="fas fa-calendar"></i> <?php echo $art['date']; ?></span>
                <span><i class="fas fa-clock"></i> <?php echo $art['read']; ?> min</span>
              </div>
              <h5><?php echo htmlspecialchars($art['title']); ?></h5>
              <p class="news-excerpt"><?php echo htmlspecialchars($art['excerpt']); ?></p>
